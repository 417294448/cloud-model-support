...
**The page's i18n / top-nav / presentation code is NOT scraped data -- `apply_update.js` preserves it, don't hand-edit the data line.** `index.html` carries hand-written features that live *outside* the `<script id="data">` blob:

- The English/中文 language toggle (`#langToggle`, the `I18N` dictionary, `t()`/`applyLang()`, all `data-i18n`/`data-i18n-ph`/`data-i18n-aria` attributes on static nodes).
- The top-right floating control bar: a single `.ctlbar` container (rounded, `backdrop-filter` blur) that wraps the three controls -- 首页/Home link to `https://www.cloudproduct.top/`, `#langToggle`, `#themeToggle`. (These used to be three independent `position:fixed` elements; they are now children of `.ctlbar`.)
- The presentation layer in `<style>` and `<head>`: the Google Fonts `<link>` (Sora / Manrope / JetBrains Mono, with `display=swap` + system-stack fallback), the font CSS variables (`--font-display`/`--font-body`/`--font-mono`), and the **semantic status-color palette** (`--c-info`/`--c-warn`/`--c-danger`/`--c-ok`/`--c-accent2`, defined per-theme in `:root` and `html.light`).
- The theme toggle handler calls `render()` after flipping the theme so status colors (offer/lifecycle/API badges, compare-section accents) are re-resolved from the now-current CSS variables via `statusColors()`/`cssVar()`. Those badge colors are **not** hardcoded in the JS anymore -- don't reintroduce `#3fb950`/`#bc8cff`-style literals when editing.
- Models with an empty region matrix (`s: {}`) — emitted by `build_azure_models.js` with a `note` explaining "no data on this page" — are **shown** in the By Model / Full Matrix views (the model filter keeps `Object.keys(mo.s).length===0` entries and `noteTag(mo)` renders the note under the name); they still don't appear in By Region / Compare Regions, which filter on region availability. Keep that split in mind if you ever touch `render()`'s view filters.

Every `apply_update.js` subcommand (`replace-models` / `replace-from-provider` / `patch`) only splices the single data line via `html.slice(0, dataStart) + JSON.stringify(data) + html.slice(dataEnd)` -- it never rewrites the markup, styles, or logic around it, so all of this survives a refresh untouched (verified: `lang-toggle`/`home-link`/`I18N`/`首页`/`t("headTitle")` counts are identical before and after `patch` and `replace-from-provider`, and `validate` passes). Three corollaries:
